...
OUTPUT_DIR="${3:-hdfs://localhost:9000/flight-delay-project/output/spark_sql/analysis_2_${DATASET_LABEL}}"
PREVIEW_PATH="${4:-results/tables_hdfs/spark_sql_analysis_2_${DATASET_LABEL}_preview.csv}"
...
spark-submit --master local[*] spark_sql/analysis_2_airport_month_delay_report.py \
  --input "$INPUT_PATH" --output "$OUTPUT_DIR"

hdfs dfs -ls "$OUTPUT_DIR"
TMP_PREVIEW_SOURCE="$(mktemp)"
trap 'rm -f "$TMP_PREVIEW_SOURCE"' EXIT
hdfs dfs -cat "$OUTPUT_DIR"/part-*.csv > "$TMP_PREVIEW_SOURCE"
