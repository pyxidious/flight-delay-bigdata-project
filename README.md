...
The benchmark is local: absolute times depend on the machine. The useful signal
is the relative scaling trend across Spark SQL, Spark Core and Hive.
...
Do not commit raw data, cleaned data, generated samples, complete job outputs or
local Hive/Spark runtime files. Small preview tables under `results/tables/`,
benchmark summaries under `results/benchmarks/`, Markdown documentation, scripts,
HiveQL queries and Spark code are intended to remain versionable.

## Planned cloud/cluster experiment

The current reproducible baseline runs locally. A future experiment will execute
the same analyses on a cloud or cluster environment and document HDFS input
handling separately. The local benchmark does not require moving inputs to HDFS.
